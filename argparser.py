...
class BufoArgParser:
    ...
    def add_arg(self, cmd_name, arg_name, type=str, required=False, default=None):
        self.arg_options[self.cmd_name_id_map[cmd_name]][arg_name] = {"type": type, "required": required, "default": default}
    ...
    def parse_args(self, input_str):
        ...
        while i < len(parts):
            if parts[i].startswith("--"):
                ...
                arg_value = self.arg_options[self.cmd_name_id_map[command]][arg_name]["type"](arg_value)
...
def main():
    ...
    assert(parser.execute(*parser.parse_args("$bufo test --arg1=1 --arg2=2 --extra_arg1=sup3r_$ecr3t_p@ssw0rd")) == "test passed")
